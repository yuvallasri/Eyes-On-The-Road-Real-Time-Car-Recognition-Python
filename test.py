...
def main():
    accident_dest = 'targetAsAccident'
    no_accident_dest = 'targetAsNoAccident'
    while 1:
        ...
        with tf.gfile.FastGFile(RETRAINED_GRAPH_PB_FILE_LOC, 'rb') as retrainedGraphFile:
            ...
            # import the graph into the current default Graph, note that we don't need to be concerned with the
            # return value
        ...
        with tf.Session() as sess:
            ...
            for fileName in os.listdir(TEST_IMAGES_DIR):
                # if the file does not end in .jpg or .jpeg (case-insensitive), continue with the next iteration of
                # the for loop
                ...
                # keep track of if we're going through the next for loop for the first time so we can show more info
                # about the first prediction, which is the most likely prediction (they were sorted descending above)
                onMostLikelyPrediction = True
                # for each prediction . . .
                for prediction in sortedPredictions:
                    strClassification = classifications[prediction]

                    # if the classification (obtained from the directory name) ends with the letter "s", remove the
                    # "s" to change from plural to singular
                    ...
                    # if we're on the first (most likely) prediction, state what the object appears to be and show a
                    # % confidence to two decimal places
                    if onMostLikelyPrediction:
                        # get the score as a %
                        scoreAsAPercent = confidence * 100.0
                        # show the result to std out, + ", " + "{0:.2f}".format(scoreAsAPercent) + "% confidence"
                        print("the object appears to be a " + strClassification)
                        # write the result on the image,  + ", " + "{0:.2f}".format(scoreAsAPercent) + "% confidence"
                        writeResultOnImage(openCVImage, strClassification)
                        # finally we can show the OpenCV image
                        # cv2.imshow(fileName, openCVImage)
                        if strClassification == "accident":
                            # mail()
                            print("call 911")
                ...
                # pause until a key is pressed so the user can see the current image (shown above) and the prediction
                # info disable for fluent run
                cv2.waitKey()
                # after a key is pressed, close the current window to prep for the next time around
                cv2.destroyAllWindows()
                if strClassification != "accident":
                    shutil.move(imageFileWithPath, accident_dest)
                if strClassification != "no accident":
                    shutil.move(imageFileWithPath, no_accident_dest)

                # os.remove('test_images/' + fileName)
            # end for
        # end with
...
def writeResultOnImage(openCVImage, resultText):
    ...
    upperLeftTextOriginX = int(imageWidth * 0.5)
    upperLeftTextOriginY = int(imageHeight * 0.90)
